    ...
    try:
        ...
        with st.spinner("Generating recommendations..."):
            ...
            if mode in ["query_with_profile", "document_with_profile", "profile_only"]:
                ...
                # Create expert profile if available or generate a default one
                ...
                # If no expert description, generate a simple default one
                if not expert_description:
                    # Extract industry from profile name (e.g., client_healthcare_01 -> healthcare)
                    industry = selected_profile.split('_')[1] if '_' in selected_profile else selected_profile
                    
                    # Create a simple default description
                    expert_description = f"Expert in {industry} industry regulations and compliance requirements in the European Union."
                    logger.info(f"Generated default expert description for {industry} industry")
                ...
                # Ensure the user profile exists in the recommender
                try:
                    # Force create the user profile if it doesn't exist
                    if user_id not in recommender._user_profiles:
                        logger.info(f"Ensuring user profile exists: {user_id}")
                        recommender._user_profiles[user_id] = recommender.get_user_profile(user_id)
                except Exception as e:
                    logger.warning(f"Error ensuring user profile exists: {str(e)}")
            else:
            ...
            if mode == "query_only":
                ...
                # Ensure we're using the correct top_k value from the sidebar
                top_k = st.session_state.get("top_k", 5)
                ...
                # Ensure we're using the correct top_k value from the sidebar
                top_k = st.session_state.get("top_k", 5)
                logger.info(f"Using top_k value for document recommendations: {top_k}")
